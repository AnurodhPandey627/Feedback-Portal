...
//Middlewares - require
const {isLoggedIn,saveRedirectUrl,isStudent,isTeacher,isCorrectStudent,isCorrectTeacher} = require("./middlewares.js");
...
const {mcqsToTranslationText,translationTextToMCQs,translateFunction} = require("./controllers/MCQController.js");
...
app.post("/newstudent", validateStudentRegistrationSchema,async (req, res) => {
  ...
  try {
    ...
    if(error.code === "ER_DUP_ENTRY"){
      throw new ExpressError(400,"Student already exists, Aadhar Number must be unique");
    }
    else{
      next(error);
    }
...
app.post("/newteacher", validateTeacherRegistrationSchema,async (req, res) => {
  ...
  try {
    ...
    console.error("Unhandled Error:", error);
    if(error.code === "ER_DUP_ENTRY"){
      next(new ExpressError(400,"Student already exists, Aadhar Number must be unique"));
    }
    else{
      next(error);
    }
...
app.post("/oldteacher", saveRedirectUrl,validateTeacherLoginSchema,async (req,res)=>{
...
//Post Student Login
app.post('/oldstudent',saveRedirectUrl,validateStudentLoginSchema, async (req, res) => {
  ...
  console.log(sclass);
  console.log(roll_no);
  const query = 'SELECT * FROM student WHERE school = ? AND class = ? AND roll_no = ?';

  try{
    const [results] =  await connection.query(query, [school, sclass, roll_no]);
...
app.get("/student/feedback",isLoggedIn,isStudent,isCorrectStudent,async (req,res)=>{
  let studentId = req.query.SID;
    ...
    try{
      const [result2] = await connection.query(q2,[DAY,P_NO]);
       //console.log(result2);//result2 is an array of objects
        ...
        //console.log(topics);
        res.render("feedback-by-student.ejs",{studentId,subjectId,subname,topics});
    }
    catch(err2){
      console.log(err2);
      next(err2);
...
app.post("/student/feedback",isLoggedIn,isStudent,isCorrectStudent,validateStudentFeedbackSchema,async (req, res,next) => {
  const { 
    SID, 
    SUBID, 
    class_start_time,
  ...
  const feedbackValuesInsertion = [SID, SUBID, class_start_time, class_end_time, understanding, interaction, practical_example, prev_hw, curr_hw, additional_feedback];
  const feedbackValuesUpdation = [class_start_time, class_end_time, understanding, interaction, practical_example, prev_hw, curr_hw, additional_feedback];
  ...
  try{
    ...
    if (results.length > 0) {
          ...
          try{
              await connection.query(updateQuery,[...feedbackValuesUpdation,SID,SUBID]);
              ...
              try{
                ...
                try{
                    const [topicRows] = await connection.query(getTFIDQuery, [topics, SUBID]);
                    const topicsValues = topicRows.map(row => [SFID, row.TOPICID, row.topic]);
                    ...
                    try{
                      ...
                      console.log("Error inserting new topics into student_feedback_topics: " + err5);
                      next(err5);
                    }
                  }
                  catch(err4){
                    console.log("Error fetching topic details from feedback-topics: "+err4);
                    next(err4);
                  }
                }
            catch(err3){
                console.log("Error deleting existing topics from student feedback: "+err3);
                next(err3);
            }
        }  
        catch(err2){
      ...
      try{
        const [result5] = await connection.query(insertQuery, feedbackValuesInsertion);
        ...
        try{
           const [results6] = await connection.query(getTFIDQuery, [SUBID]);
            ...
            try{
                const [results7] = await connection.query(getTopicIDsQuery, [TFID, topics]);
                
                //Prepare insertion values
                const topicInsertValues = results7.map(row => [SFID, row.TOPICID, row.topic]);
                ...
                console.log('error inserting topics in student_feedback_toics: '+err9);
                next(err9);
               }
            }
            catch(err8){
...
app.get("/student/questionnaire",isLoggedIn,isStudent,isCorrectStudent,wrapAsync(async (req, res,next) => {
...
app.post("/student/questionnaire", isLoggedIn,isStudent,isCorrectStudent,validateStudentQuestionnaireSchema,wrapAsync (async (req, res) => {
  const  SFID  = req.query.SFID;
  ...
  const correctAnswerArray = Array.isArray(correct_answers) ? correct_answers : [correct_answers];

  // Prepare question values for bulk insert
  const questionValues = questionArray.map((q, i) => [q, correctAnswerArray[i]]);

  const insertQuestionsQuery = `INSERT INTO question (question_text, correct_answer) VALUES ?`;

  const [result1] = await connection.query(insertQuestionsQuery, [questionValues]);
  
    const insertedIds = Array.from({ length: result1.affectedRows }, (_, i) => result1.insertId + i);

    const studentAnswerValues = insertedIds.map((qid, i) => [SFID, qid, answerArray[i]]);
...
//Student Dashboard
app.get("/student/:id",isLoggedIn,isStudent,isCorrectStudent, wrapAsync(async (req,res)=>{
  let {id} = req.params;
  //console.log(`${id}`);
...
app.get("/teacher-schedule", isLoggedIn,isTeacher,isCorrectTeacher,wrapAsync(async (req, res) => {
  ...
  const [scheduleResults] = await connection.query(scheduleQuery, [TID, currentDay]);
    ...
    const [feedbackResults] = await connection.query(feedbackQuery, [TID, todayDate]);
...
app.get("/teacher/feedback",isLoggedIn,isTeacher,isCorrectTeacher,(req,res)=>{
  let TID = req.query.TID;
...
//TEACHER'S DASHBOARD
app.get("/teacher/:id",isLoggedIn,isTeacher,isCorrectTeacher,wrapAsync(async (req,res)=>{
    let {id} = req.params;
    if (!id || isNaN(id)) {
...
app.post("/teacher/feedback", isLoggedIn,isTeacher,isCorrectTeacher,validateTeacherFeedbackSchema,async (req, res, next) => {
  try {
    const {
      TID,
      SUBID,
    ...
    const feedbackValues = [TID, SUBID, DAY, P_NO, class_start_time, class_end_time, present_students, additional_feedback, rating];
